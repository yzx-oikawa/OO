...
public class Taxi extends Thread{
	 ...
	 * 抽象函数：AF(c)=(No, state, credit, x, y, lastx, lasty, order, map, gui, road, light)  
	         where No==c.No, state==c.state, credit==c.credit, x==c.x, y==c.y, lastx==c.lastx, lasty==c.lasty,
	         	   order==c.order, map==c.map, gui==c.gui, road==c.road, light==c.light
	 * 不变式：0<=c.No<=99 && 1<=c.state<=4 && c.credit>=0 && 0<=c.x,c.y,c.lastx,c.lasty<80 && 
	...
	public boolean repOK()
	{
		...
		*/
		if(No<0 || No>99 || state<1 || state>4 || credit<0)
			return false;
